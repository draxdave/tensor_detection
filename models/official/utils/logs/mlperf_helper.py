...
"""Wrapper for the mlperf m_util utils.

MLPerf compliance m_util is only desired under a limited set of circumstances.
This module is intended to keep users from needing to consider m_util (or
...
# TODO(robieta): move line parsing to mlperf m_util
...
class Logger(object):
  ...
  def __call__(self, enable=False):
    if enable and self._mlperf_log is None:
      raise ImportError("MLPerf m_util was requested, but mlperf_compliance "
