...
# Dictionary containing tensors that are logged by the m_util hooks. Each item
...
def model_fn(features, labels, mode, params):
  ...
  with tf.variable_scope("model"):
    ...
    # Save loss as named tensor that will be logged with the m_util hook.
...
def get_learning_rate(learning_rate, hidden_size, learning_rate_warmup_steps):
  ...
  with tf.name_scope("learning_rate"):
    ...
    # Create a named tensor that will be logged using the m_util hook.
...
def run_transformer(flags_obj):
  ...
  train_hooks = hooks_helper.get_train_hooks(
      flags_obj.hooks,
      model_dir=flags_obj.model_dir,
      tensors_to_log=TENSORS_TO_LOG,  # used for m_util hooks
